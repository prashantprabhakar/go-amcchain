...
// Package gethclient provides an RPC client for amcth-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements amcth-specific functionality.
//
// If you want to use the standardized AmcChain RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
// GCStats retrieves the current garbage collection stats from a amcth node.
...
// MemStats retrieves the current memory stats from a amcth node.
...
// GetNodeInfo retrieves the node info of a amcth node.
