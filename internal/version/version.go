...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "amcth".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("amcth %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("amcth %s", params.VersionWithMeta)
	}
