...
This setup allows a DApp to connect to a remote AmcChain node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote AmcChain node, because a local one is not available, not synchronized with the chain, or is a node that has no built-in (or limited) account management.
...
In this case, `amcth` would be started with `--signer http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
* Geth todos
    ...
    - Relay: Geth should be started in `amcth --signer localhost:8550`.
