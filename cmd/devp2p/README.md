...
Use `devp2p enrdump <base64>` to verify and display an AmcChain Node Record.
...
1. initialize the amcth node with the `genesis.json` file
2. import blocks from `chain.rlp`
3. run the client using the resulting database. For amcth, use a command like the one below:
...
[dns-tutorial]: https://amcth.ethereum.org/docs/developers/amcth-developer/dns-discovery-setup
