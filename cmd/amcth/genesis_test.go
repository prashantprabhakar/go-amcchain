...
func TestCustomGenesis(t *testing.T) {
	...
	for i, tt := range customGenesisTests {
		...
		amcth := runGeth(t, "--networkid", "1337", "--syncmode=full", "--cache", "16",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0", "--authrpc.port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		amcth.ExpectRegexp(tt.result)
		amcth.ExpectExit()
	}
}
...
func TestCustomBackend(t *testing.T) {
	...
	testfunc := func(t *testing.T, tt backendTest) error {
		...
		{ // Init
			...
			amcth := runGeth(t, args...)
			amcth.ExpectRegexp(tt.initExpect)
			amcth.ExpectExit()
		}
		{ // Exec + query
			...
			amcth := runGeth(t, args...)
			amcth.ExpectRegexp(tt.execExpect)
			amcth.ExpectExit()
		}
		return nil
	}
	for i, tt := range []backendTest{
		...
		{ // Can't start pebble on top of leveldb
			initArgs:   []string{"--db.engine", "leveldb"},
			execArgs:   []string{"--db.engine", "pebble"},
			execExpect: `Fatal: Failed to register the AmcChain service: db.engine choice was pebble but found pre-existing leveldb database in specified data directory`,
		},
		{ // Can't start leveldb on top of pebble
			initArgs:   []string{"--db.engine", "pebble"},
			execArgs:   []string{"--db.engine", "leveldb"},
			execExpect: `Fatal: Failed to register the AmcChain service: db.engine choice was leveldb but found pre-existing pebble database in specified data directory`,
		},
