...
import (
	...
	"github.com/prashantprabhakar/go-amcchain/crypto/signify"
	"github.com/prashantprabhakar/go-amcchain/internal/build"
	"github.com/prashantprabhakar/go-amcchain/params"
)

var (
	// Files that end up in the amcth*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("amcth"),
	}

	// Files that end up in the amcth-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert AmcChain contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "AmcChain bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (AmcChain Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "amcth",
			Description: "AmcChain CLI client.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "AmcChain account management tool.",
		},
	}
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	var ld []string
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/prashantprabhakar/go-amcchain/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/prashantprabhakar/go-amcchain/internal/version.gitDate="+env.Date)
	}
...
func doArchive(cmdline []string) {
	...
	var (
		...
		amcth    = "amcth-" + basegeth + ext
		alltools = "amcth-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(amcth, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{amcth, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with amcth-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "amcth-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "amcth-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "AmcChain Builds <fjl@ethereum.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "amcth.exe" {
			gethTool = file
		} else {
	...
	// first section contains the amcth binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.amcth.nsi", filepath.Join(*workdir, "amcth.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("amcth-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "amcth.nsi"),
	)
